...
class Model_SPEC2MIDI(nn.Module):
    ...
    def forward(self, input_spec):
        ...
        (
            ...
            output_mpe_B,
            output_velocity_B,
        ) = self.decoder_spec2midi(enc_vector)
...
class Encoder_SPEC2MIDI(nn.Module):
    ...
    def forward(self, spec_in):
        ...
        spec_freq = self.dropout(
            (spec_emb_freq * self.scale_freq) + self.pos_embedding_freq(pos_freq)
        )
...
class Decoder_SPEC2MIDI(nn.Module):
    def __init__(
        ...
        self.layer_zero_freq = DecoderLayer_Zero(
    ...
    def forward(self, enc_spec):
        ...
        midi_time = self.dropout(
            (midi_time * self.scale_time) + self.pos_embedding_time(pos_time)
        )
