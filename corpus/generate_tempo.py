...
def load_beat_pred(filename):
    with open(filename, 'r', encoding='utf-8') as f:
        data = json.load(f)
    return data['beat_pred'], data['downbeat_pred']

def save_json(filename, events):
    with open(filename, 'w', encoding='utf-8') as f:
        json.dump(events, f, indent=4, ensure_ascii=False)

def remove_close_beats(beats, downbeats, beat_threshold=0.1):
    """
    去除與 downbeat 時間過於接近的 beat，避免重複計算
    """
    filtered = []
    for beat in beats:
        if any(abs(beat - db) < beat_threshold for db in downbeats):
            continue
        filtered.append(beat)
    return filtered

# === 小節資訊計算 ===
def compute_measures(beats, downbeats, uniformity_threshold=0.1):
    """
...
def compute_global_time_sig(measures):
    """
...
# === 穩定區域檢測 ===
def detect_stable_regions(measures, window_size=4, threshold=0.1):
    """
...
def partition_unstable_regions(total_measures, stable_regions):
    """
...
# === 不穩定區域調整 ===
def adjust_unstable_region(measures, un_region, prev_stable_idx, next_stable_idx, global_time_sig, epsilon=0.1):
    """
...
# === tempo 事件生成 ===
def generate_tempo_events(measures, stable_regions, global_time_sig):
    """
...
def generate_region_events_from_events(events):
    """
...
def events_to_beat_pred(region_events, default_time_sig=4, default_bpm=120):
    """
...
def play_audio_with_clicks(audio_path, beat_pred_data):
    """
...
def generate_tempo_info(base_folder):
    ...
    # 讀取 beat 與 downbeat
    beats, downbeats = load_beat_pred(beat_pred_file)
    print("[DEBUG] 原始 beat 數量 =", len(beats))
    print("[DEBUG] 原始 downbeat 數量 =", len(downbeats))
    
    # 去除與 downbeat 接近的 beat
    filtered_beats = remove_close_beats(beats, downbeats, beat_threshold=0.1)
    print("[DEBUG] 篩選後 beat 數量 =", len(filtered_beats))
    
    # 初步計算小節資訊
    measures = compute_measures(filtered_beats, downbeats)
    print(f"[DEBUG] 初步計算出 {len(measures)} 個小節")
    
    # 決定全局拍號，並更新各 measure 的 BPM
    global_time_sig = compute_global_time_sig(measures)
    
    # 檢測穩定區域（至少4個小節）
    stable_regions = detect_stable_regions(measures, window_size=4, threshold=0.1)
    
    # 分割出不穩定區域
    unstable_regions = partition_unstable_regions(len(measures), stable_regions)
    ...
    for un_region in unstable_regions:
        ...
        for reg in stable_regions:
            if reg[0] > end_un:
                ...
                break
        adjust_unstable_region(measures, un_region, prev_stable_idx, next_stable_idx, global_time_sig, epsilon=0.1)
    ...
    # 生成 tempo 事件
    events = generate_tempo_events(measures, stable_regions, global_time_sig)
    region_events = generate_region_events_from_events(events)
    beat_pred_data = events_to_beat_pred(region_events)
    
    # 輸出 tempo.json
    save_json(tempo_output_file, region_events)
    print(f"tempo.json 已生成，共 {len(region_events)} 條事件。")
    save_json(beat_pred_output_file, beat_pred_data)